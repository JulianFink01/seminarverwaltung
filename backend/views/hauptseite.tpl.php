...
<body>
	...
	<main id="hintergrund">
		...
		<div id="inhalt">
			...
			?>
				<div id="textdeco">
					<div class="f_inhalt<?php
										if ($fortbildung->getStatus() == 0) {
											echo ' disabled';
										}
										?>">
						<a href="index.php?aktion=alle_kurse&fortbildung_id=<?php
						...
						<a href="?aktion=loescheFortbildung&fortbildung_id=<?php
																			echo $fortbildung->getId();
																			?>" id="loesche_f">
							<img class="kurs_icons" width="35px" src="images/muelleimer_icon.png" title="löschen" />
						</a>
						<a href="?aktion=duplicateFortbildung&fortbildung_id=<?php
																				echo $fortbildung->getId();
																				?>" id="duplicate_f">
							<img class="kurs_icons" width="35px" src="images/clon_icon.png" title="duplizieren" />
						</a>
						<a href="?aktion=statusAendern&fortbildung_id=<?php
																		echo $fortbildung->getId();
																		?>" id="duplicate_f">
							<img class="kurs_icons" width="35px" src="<?php
																		echo $img_status;
...
<script type="text/javascript">
	function triggerTextfeld() {
		const field = document.getElementById("kurs-add");
		field.classList.toggle("show_name");
		document.getElementById("gesamt").classList.toggle("blur");
	}

	function closePopUp() {
		const field = document.getElementById("titel-aendern");
		field.classList.toggle("show_name");
		document.getElementById("gesamt").classList.toggle("blur");
	}
	...
	function bearbeiteName(titel, fid) {
		const field = document.getElementById("titel-aendern");
		const form = document.getElementById("t_aendern_form");
		field.classList.toggle("show_name");

		const l = document.createElement("LEGEND");
		const t = document.createTextNode("Veranstaltungstitel ändern");
		l.appendChild(t);
