...
<body>
	<header id="kopf">
		<div id="home-redi"><a href="?aktion=hauptseite">
		...
		<h1>Kursverwaltung - <?php
								echo $fortbildung->getName()
								?></h1>
	...
	<div id="leiste">
		<article class="infobox">
			<section id="allgemeiner" class="tab activeTab">
				<h2>
					<a href="#allgemeiner" onclick="toggleTab(document.getElementById('allgemeiner'))">Kurse</a>
				</h2>
					...
					?>
							<div class="kurs">
								<h1><?php
									echo $kurs->getTitel();
									?></h1>
								<a href="?aktion=kurs_bearbeiten&fortbildung_id=<?php
								...
								<a href="?aktion=loesche&fortbildung_id=<?php
																		echo $_REQUEST["fortbildung_id"]
																		?>&kurs_id=<?php
																					echo $kurs->getId()
																					?>">
								...
								<a href="?aktion=teilnehmerliste&kurs_id=<?php
																			echo $kurs->getId()
																			?>" target="drucken">
									<img class="kurs_icons" width="35px" src="images/personen.png" title="Teilnehmerliste" />
								</a>
								<a href="?aktion=duplicateKurs&fortbildung_id=<?php
																				echo $fortbildung->getId();
																				?>&kurs_id=<?php
																							echo $kurs->getId()
																							?>" id="duplicate_f">
				...
				<div id="kurs_erstellbutton">
					<a href="index.php?aktion=kurse_erstellen&fortbildung_id=<?php
																				echo $_REQUEST['fortbildung_id']
																				?>#allgemeiner">
			...
			<section id="funktionen" class="tab tabElem">
				...
				<div class="csv">
					<form method="post" enctype="multipart/form-data" action="index.php?aktion=import_lehrer&fortbildung_id=<?php
																															echo $_REQUEST['fortbildung_id']
																															?>#funktionen">
						<label>
				...
				<table id="teilnehmer-tabelle">
					...
					?>
						<tr>
							<td><?php
								echo $teilnehmer->getVorname();
								?></td>
							<td><?php
								echo $teilnehmer->getNachname();
								?></td>
							<td><?php
								echo $teilnehmer->getEmail();
								?></td>
							<td><?php
								echo $teilnehmer->getToken();
								?></td>
							<td class="hidden"><?php
												if (NimmtTeil::findeNachFortbildungUndTeilnehemer($fortbildung, $teilnehmer)->getStatusFarbe() == 'blue') {
													echo 'A';
												} else {
													echo 'B';
												}
												?></td>
							<td style="background-color: const(--main-<?php
																		echo NimmtTeil::findeNachFortbildungUndTeilnehemer($fortbildung, $teilnehmer)->getStatusFarbe();
																		?>);">
								&nbsp;
							</td>
							<td class="b_l">
								<a href="index.php?aktion=remove_lehrer_nimmtTeil&teilnehmer_id=<?php
																								echo $teilnehmer->getId()
																								?>&fortbildung_id=<?php
																													echo $_REQUEST['fortbildung_id']
																													?>#funktionen">
							...
							<td class="b_l">
								<a onclick="bearbeiteBenutzer('<?php echo $teilnehmer->getToken(); ?>', 
								'<?php echo $teilnehmer->getVorname(); ?>',
								'<?php echo $teilnehmer->getNachname(); ?>',
								'<?php echo $teilnehmer->getEmail(); ?>',
								'<?php echo $_REQUEST['fortbildung_id'] ?>')">
			...
			<section id="emailsenden" class="tab tabElem">
				...
				<div id="fenster">
					...
					<form action="index.php?aktion=send_email&fortbildung_id=<?php
																				echo $_REQUEST['fortbildung_id']
																				?>" method="post">
...
<script type="text/javascript">
	function teilnehmerErstellen() {
		const textfeld = document.getElementById("teilnehmer-erstellen");
	...
	function closePopUp() {
		const field = document.getElementById("teilnehmer-bearbeiten");
	...
	function bearbeiteBenutzer(token, vorname, nachname, email, fortbildung_id) {
		const field = document.getElementById("teilnehmer-bearbeiten");
		const form = document.getElementById("t_bearbeiten_form");
		field.classList.toggle("show_teilnehmer");

		const l = document.createElement("LEGEND");
		const t = document.createTextNode("Teilnehmer bearbeiten");
		l.appendChild(t);
