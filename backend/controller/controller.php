...
class Controller
{
	...
	{
		$alteFortbildung = Fortbildung::finde(
			$_GET['fortbildung_id']
		);
	...
	public function duplicateKurs()
	{
		...
		header('Location: index.php?aktion=alle_kurse&fortbildung_id='
			. $_GET["fortbildung_id"] . '#allgemeiner');
	}
	...
	public function teilnehmer_bearbeiten()
	{
		...
		header(
			'Location: index.php?aktion=alle_kurse&fortbildung_id='
				. $_POST["fortbildung_id"] . '#funktionen'
		);
	}
	...
	public function titel_aendern()
	{
		...
		echo $name;
		echo $_POST['fid'];
		if ($name != '' && $name != ' ') $fort->setName($name);
		$fort->speichere();
	...
	public function loesche()
	{
		...
		$kurs->loesche();
		header('Location: index.php?aktion=alle_kurse&fortbildung_id='
			. $_REQUEST["fortbildung_id"] . '#allgemeiner');
	}
	...
	public function saveTeilnehmer()
	{
		...
		header('Location: index.php?aktion=alle_kurse&fortbildung_id='
			. $_REQUEST['fortbildung_id'] . '#funktionen');
	}
